...
namespace NBitcoin
{
    ...
    public class ExtKey : IBitcoinSerializable, IDestination, ISecret
    {
        ...
        private static byte[] hashkey = null; // = Encoders.ASCII.DecodeData("zdrgrewtht seed");
        ...
        private void SetMaster(byte[] seed)
        {
            ...
            if (hashkey == null || hashkey.Length < 9)
            {

                throw new ArgumentException(string.Format("The salt code must be at least 8 bytes."), "salt");
        ...
        public ExtKey(byte[] seed, byte[] inhashkey)
        {

            hashkey = inhashkey;
            SetMasterSafe(seed.ToArray(), hashkey);
        ...
        private void SetMasterSafe(byte[] seed, byte[] ihashkey)
        {
            ...
            var hashMAC = Hashes.HMACSHA512(ihashkey, seed);
            key = new Key(hashMAC.SafeSubarray(0, 32));

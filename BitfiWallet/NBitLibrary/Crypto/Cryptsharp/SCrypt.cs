...
namespace NBitcoin.Crypto
{
    ...
    public static class SCrypt
    {
                      ...
                      int cost, int blockSize, int parallel, int maxThreads)
        {
            ...
            int threadCount = Math.Max(1, Math.Min(4, Math.Min(maxThreads, parallel)));
      ...
      Task[] threads = new Task[threadCount - 1];
        ...
        {
            return Crypto.SCrypt.ComputeDerivedKey(password, salt, 16384, 8, 8, 4, outputCount);
        }

        public static byte[] BitcoinComputeDerivedKey2(byte[] password, byte[] salt, int outputCount = 64)
        {
            return Crypto.SCrypt.ComputeDerivedKey(password, salt, 1024, 1, 1, 1, outputCount);
        }
